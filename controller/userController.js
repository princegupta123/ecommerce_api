...
export const addToCart = async (req, res) => {
  try {
    ...
    let cartItem = await Cart.findOne({ where: { pId:productId, userId: userId } });
    if (cartItem) {
      cartItem = await Cart.update({ quantity: quantity }, { where: { pId: productId, userId: userId } });
    } else {
      cartItem = await Cart.create({ pId: productId, quantity:quantity, userId:userId });
    }
...
export const deleteCartItem=async(req, res)=>{
    try {
        ...
        const existingCartItem = await Cart.findOne({where:{
            pId: productId,
